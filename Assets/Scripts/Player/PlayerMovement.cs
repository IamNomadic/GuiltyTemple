...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else if ((canMove && horizontal > 0.1 && isJumping == false && playerCombat.isAttacking && HTransformed) ||
                 (canMove && horizontal < -0.1 && isJumping == false && playerCombat.isAttacking && HTransformed))
            animator.Play("WalkingAttack");
        else if ((canMove && horizontal < 0.1 && playerCombat.isAttacking && HTransformed) ||
                 (canMove && horizontal > -0.1 && playerCombat.isAttacking && HTransformed))
            animator.Play("Attack");
        else if (canMove && isJumping && HTransformed && !isDodging)
            animator.Play("Jump");
        else if ((canMove && horizontal > 0.1 && isJumping == false && !isDodging && HTransformed) ||
                 (canMove && horizontal < -0.1 && isJumping == false && !isDodging && HTransformed))
            animator.Play("Run");
        else if ((canMove && horizontal < 0.1 && isJumping == false && !isDodging && !playerCombat.isAttacking &&
                  HTransformed) ||
                 (canMove && horizontal > -0.1 && isJumping == false && !isDodging && !playerCombat.isAttacking &&
                  HTransformed)) animator.Play("Stand");
        if (canMove && isDodging && HTransformed) animator.Play("Dash");
        ///vampire anims
        if (canMove && isJumping && playerCombat.isAttacking && VTransformed)
            animator.Play("VAirAttack");
        else if ((canMove && horizontal > 0.1 && isJumping == false && playerCombat.isAttacking && VTransformed) ||
                 (canMove && horizontal < -0.1 && isJumping == false && playerCombat.isAttacking && VTransformed))
            ...
            animator.Play("VJump");
        else if ((canMove && horizontal > 0.1 && isJumping == false && !isDodging && VTransformed) ||
                 (canMove && horizontal < -0.1 && isJumping == false && !isDodging && VTransformed))
            animator.Play("VWalk");
        else if ((canMove && horizontal < 0.1 && isJumping == false && !isDodging && !playerCombat.isAttacking &&
                  VTransformed) ||
                 (canMove && horizontal > -0.1 && isJumping == false && !isDodging && !playerCombat.isAttacking &&
                  VTransformed)) animator.Play("VStand");
        if (canMove && isDodging && VTransformed) animator.Play("VDash");
        /// wolf anims
        if (canMove && isJumping && playerCombat.isAttacking && WTransformed)
            animator.Play("WAirAttack");
        else if ((canMove && horizontal > 0.1 && isJumping == false && playerCombat.isAttacking && WTransformed) ||
                 (canMove && horizontal < -0.1 && isJumping == false && playerCombat.isAttacking && WTransformed))
            ...
            animator.Play("WJump");
        else if ((canMove && horizontal > 0.1 && isJumping == false && !isDodging && WTransformed) ||
                 (canMove && horizontal < -0.1 && isJumping == false && !isDodging && WTransformed))
            animator.Play("WWalk");
        else if ((canMove && horizontal < 0.1 && isJumping == false && !isDodging && !playerCombat.isAttacking &&
                  WTransformed) ||
                 (canMove && horizontal > -0.1 && isJumping == false && !isDodging && !playerCombat.isAttacking &&
                  WTransformed)) animator.Play("WStand");
        if (canMove && isDodging && WTransformed) animator.Play("WDash");
        ...
        if (canMove)
        {
            if (!isDodging) rb.velocity = new Vector2(horizontal * walkSpeed, rb.velocity.y);

            if (isDodging)
            {
                if (!isFacingRight)
                    rb.velocity = new Vector2(-dodgeSpeed, rb.velocity.y);
                else if (isFacingRight) rb.velocity = new Vector2(dodgeSpeed, rb.velocity.y);
            }
        }
        ...
        if (canMove)
            if (IsGrounded() && isJumping)
            {
                ...
                else if (WTransformed) animator.Play("WStand");
    ...
    [SerializeField] private float vWalkSpeed;
    ...
    [SerializeField] private float wWalkSpeed;
    ...
    [SerializeField] private float hJumpPower;
    [SerializeField] private float hDodgeSpeed;
    [SerializeField] private float hAttackRange;
